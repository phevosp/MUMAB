...
class LogMultiAgentInteraction(MultiAgentInteractionInterface):
    ...
    def add_constraints(self, m, store_vars):
        ...
        store_vars[f"x_{self.arm_id}"] = m.addVar(
            vtype=gp.GRB.INTEGER, lb=0.0, ub=self.M, name=f"x_{self.arm_id}"
        )
        ...
        store_vars[f"f(x_{self.arm_id})"] = m.addVar(
            vtype=gp.GRB.CONTINUOUS,
            lb=0,
            ub=(
                np.emath.logn(self.log_base, self.alpha * self.M + 1 / (self.log_base))
                + 1
            )
            / (np.emath.logn(self.log_base, self.alpha + 1 / (self.log_base)) + 1),
        ...
        # Add constraints
        m.addConstr(
            temp1 == self.alpha * store_vars[f"x_{self.arm_id}"] + 1 / self.log_base,
            name=f"constr1_x_{self.arm_id}",
        )
        m.addGenConstrLogA(temp1, temp2, self.log_base)
        m.addConstr(
            store_vars[f"f(x_{self.arm_id})"]
            == (temp2 + 1)
            / (np.emath.logn(self.log_base, self.alpha + 1 / self.log_base) + 1),
            name=f"constr2_x_{self.arm_id}",
        )
...
class LinearMultiAgentInteraction(MultiAgentInteractionInterface):
    ...
    def add_constraints(self, m, store_vars):
        ...
        store_vars[f"f(x_{self.arm_id})"] = m.addVar(
            vtype=gp.GRB.CONTINUOUS, name=f"f(x_{self.arm_id})"
        )

        # Constrain transformation == number of agents
        m.addConstr(
            store_vars[f"f(x_{self.arm_id})"] == store_vars[f"x_{self.arm_id}"],
            name=f"constr_x_{self.arm_id}",
        )
...
class ConstantMultiAgentInteraction(MultiAgentInteractionInterface):
    ...
    def add_constraints(self, m, store_vars):
        ...
        m.addConstr(
            store_vars[f"x_{self.arm_id}"] >= eps - M * (1 - b),
            name=f"constr_x_{self.arm_id}_b=1",
        )
        # If b = 0, x <= 0, so x = 0
        # If b = 1, this is non-binding
        m.addConstr(
            store_vars[f"x_{self.arm_id}"] <= M * b, name=f"constr_x_{self.arm_id}_b=0"
        )

        # If b == 1, then constrain f(x) == 1
        # If b == 0, then constrain f(x) == x == 0
        m.addConstr(
            (b == 1) >> (store_vars[f"f(x_{self.arm_id})"] == 1),
            name=f"constr_f(x_{self.arm_id})_b=1",
        )
        m.addConstr(
            (b == 0)
            >> (store_vars[f"f(x_{self.arm_id})"] == store_vars[f"x_{self.arm_id}"]),
            name=f"constr_f(x_{self.arm_id})_b=0",
        )
...
class CollisionMultiAgentInteraction(MultiAgentInteractionInterface):
    ...
    def add_constraints(self, m, store_vars):
        ...
        # If b = 0, this is non-binding
        m.addConstr(
            store_vars[f"x_{self.arm_id}"] >= 1 + eps - M * (1 - b),
            name=f"constr_x_{self.arm_id}_b=1",
        )
        # If b = 0, then x <= 1
        # If b = 1, then this is non-binding
        m.addConstr(
            store_vars[f"x_{self.arm_id}"] <= 1 + M * b,
            name=f"constr_x_{self.arm_id}_b=0",
        )

        # Add constraints
        m.addConstr(
            (b == 1) >> (store_vars[f"f(x_{self.arm_id})"] == 0),
            name=f"constr_f(x_{self.arm_id})_b=1",
        )
        m.addConstr(
            (b == 0)
...
class PowerMultiAgentInteraction(MultiAgentInteractionInterface):
    ...
    def __init__(self, id, M, numer=1, denom=2):
        try:
            ...
            raise ValueError(
                "Numerator must be non-negative and less than the denominator"
            )
