...
class Manager:
    ...
    def _evaluate_type(
        ...
        for trial in range(self.params.num_trials):
            ...
            reward_per_turn, curr_time, transition_intervals, episode_allocations = (
                mab_alg.run(f)
            )
            ...
            # Calculate regret
            cum_regret = np.subtract(
                np.array([max_reward_per_turn * i for i in range(1, self.T + 1)]),
                np.cumsum(reward_per_turn),
            )
            ...
            plt.plot_transition_regret_per_episode_cost(
                reward_per_turn,
                max_reward_per_turn,
                transition_intervals,
                trial_output_dir,
    ...
    def evaluate_algs(self, output_dir, regret, ftype):
        ...
        for name, type in zip(self.params.alg_names, self.params.alg_types):
            ...
            if not type in regret:
                regret[type] = {}

            # Call evaluate_type on specific algorithm
            regret[type][ftype] = self._evaluate_type(
                max_reward_per_turn,
                max_regret_per_turn,
                type,
                name,
            ...
            plt.plot_cumulative_regret_total(
                self.cumulative_regrets[type],
                regret[type][ftype],
                output_dir_type,
                self.T,
                self.params.normalized,
            )
            plt.plot_average_regret_total(
                self.cumulative_regrets[type],
                regret[type][ftype],
                output_dir_type,
                self.T,
        ...
        plt.plot_algs_cum_regret(
            self.cumulative_regrets,
            self.params.alg_names,
            self.params.alg_types,
            output_dir,
            self.T,
            self.params.normalized,
        )
...
def plot_function_regrets(params, regret):
    for alg_name, alg_type in zip(params.alg_names, params.alg_types):
        plt.plot_algs_avg_regret_ftypes(
            regret[alg_type],
            params.function_types,
            alg_type,
            alg_name,
            params.T,
            params.output_dir,
        ...
        plt.plot_algs_cum_regret_ftypes(
            regret[alg_type],
            params.function_types,
            alg_type,
            alg_name,
