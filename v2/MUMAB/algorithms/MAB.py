...
class MAB_indv:
    ...
    def _episode(self, agents, curr_time):
        ...
        for agent in agents:
            ...
            G_directed = nx.DiGraph(self.G)
            for u, v in self.G.edges():
                G_directed.edges[u, v]["weight"] = (
                    max_ucb - agent.ucb_dict[self.G.nodes[v]["arm"].id]
                )
                G_directed.edges[v, u]["weight"] = (
                    max_ucb - agent.ucb_dict[self.G.nodes[u]["arm"].id]
                )

            # Compute single source shortest path and add to dictionary
            paths[agent.id] = nx.shortest_path(
                G_directed,
        ...
        baseline_pulls = min(
            [agent.num_pulls_dict[paths[agent.id][-1]] for agent in agents]
        )
        baseline_agent = [
            agent
            for agent in agents
            if agent.num_pulls_dict[paths[agent.id][-1]] == baseline_pulls
        ][0]
        ...
        while (
            baseline_agent.num_pulls_dict[baseline_arm] < 2 * baseline_pulls
            and curr_time < self.T
        ):
            curr_time += 1
            rew_per_turn.append(self._step(curr_time, agents, arm_dict))
    ...
    def run(self, f):
        ...
        # Initialize agents and assign vertex
        agents = [
            Agent(i, self.G.nodes[random.randint(0, self.K - 1)], self.G)
            for i in range(self.M)
        ]
